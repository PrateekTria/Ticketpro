...
public final class LPRCaptureActivity extends Activity implements SurfaceHolder.Callback, 
  ShutterButton.OnShutterButtonListener, CloseButton.OnCloseButtonListener {

  private static final String TAG = LPRCaptureActivity.class.getSimpleName();
  ...
  void onShutterButtonPressContinuous() {
    ...
    if (lastResult != null) {
      ...
      Toast toast = Toast.makeText(this, "LPR failed. Please try again.", Toast.LENGTH_SHORT);
  ...
  */
  ...
  private File getStorageDirectory() {
    ...
    if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
      ...
      //          // "/mnt/sdcard/Android/data/com.ticket.pro.lpr/files/"
  ...
  private void initOcrEngine(File storageRoot, String languageCode, String languageName) {    
    ...
    if (ocrEngineModeName.equals("Both")) {
      indeterminateDialog.setMessage("Initializing Cube and Tesseract LPR engines for " + languageName + "...");
    } else {
      indeterminateDialog.setMessage("Initializing " + ocrEngineModeName + " LPR engine for " + languageName + "...");
    }
    ...
    new OcrInitAsyncTask(this, baseApi, dialog, indeterminateDialog, 
    		languageCode, languageName, ocrEngineMode).execute(storageRoot.toString());
  }
  ...
  boolean handleOcrDecode(OcrResult ocrResult) {
    ...
    // TODO - Return Result NEED TO CHANGE
    Intent data = new Intent();
	data.putExtra("PlateNumber", ocrResult.getText());
	if(getParent() == null) {
    ...
    if (lastBitmap == null) {
      bitmapImageView.setImageBitmap(BitmapFactory.decodeResource(getResources(),R.drawable.logo_small));
    } else {
  ...
  void handleOcrContinuousDecode(OcrResult ocrResult) {
    ...
    if (CONTINUOUS_DISPLAY_METADATA) {
      ...
      statusViewBottom.setText("LPR: " + sourceLanguageReadable + " - Mean confidence: " + 
  ...
  void handleOcrContinuousDecode(OcrResultFailure obj) {
    ...
    if (CONTINUOUS_DISPLAY_METADATA) {
      ...
      statusViewBottom.setTextSize(14);
      CharSequence cs = setSpanBetweenTokens("LPR: " + sourceLanguageReadable + " - LPR failed - Time required: " 
  ...
  void showLanguageName() {   
    Toast toast = Toast.makeText(this, "LPR: " + sourceLanguageReadable, Toast.LENGTH_LONG);
    toast.setGravity(Gravity.TOP, 0, 0);
    toast.show();
  }
  ...
  void setStatusViewForContinuous() {
    viewfinderView.removeResultText();
    if (CONTINUOUS_DISPLAY_METADATA) {
      statusViewBottom.setText("LPR: " + sourceLanguageReadable + " - waiting for LPR...");
    }
  }
  ...
  private void retrievePreferences() {
      ...
      setSourceLanguage(prefs.getString(PreferencesActivity.KEY_SOURCE_LANGUAGE_PREFERENCE, LPRCaptureActivity.DEFAULT_SOURCE_LANGUAGE_CODE));
      setTargetLanguage(prefs.getString(PreferencesActivity.KEY_TARGET_LANGUAGE_PREFERENCE, LPRCaptureActivity.DEFAULT_TARGET_LANGUAGE_CODE));
      isTranslationActive = prefs.getBoolean(PreferencesActivity.KEY_TOGGLE_TRANSLATION, false);
      
      // Retrieve from preferences, and set in this Activity, the capture mode preference
      if (prefs.getBoolean(PreferencesActivity.KEY_CONTINUOUS_PREVIEW, LPRCaptureActivity.DEFAULT_TOGGLE_CONTINUOUS)) {
  ...
  private void setDefaultPreferences() {
    ...
    // Continuous preview
    prefs.edit().putBoolean(PreferencesActivity.KEY_CONTINUOUS_PREVIEW, LPRCaptureActivity.DEFAULT_TOGGLE_CONTINUOUS).commit();

    // Recognition language
    prefs.edit().putString(PreferencesActivity.KEY_SOURCE_LANGUAGE_PREFERENCE, LPRCaptureActivity.DEFAULT_SOURCE_LANGUAGE_CODE).commit();

    // Translation
    prefs.edit().putBoolean(PreferencesActivity.KEY_TOGGLE_TRANSLATION, LPRCaptureActivity.DEFAULT_TOGGLE_TRANSLATION).commit();

    // Translation target language
    prefs.edit().putString(PreferencesActivity.KEY_TARGET_LANGUAGE_PREFERENCE, LPRCaptureActivity.DEFAULT_TARGET_LANGUAGE_CODE).commit();

    // Translator
    prefs.edit().putString(PreferencesActivity.KEY_TRANSLATOR, LPRCaptureActivity.DEFAULT_TRANSLATOR).commit();

    // OCR Engine
    prefs.edit().putString(PreferencesActivity.KEY_OCR_ENGINE_MODE, LPRCaptureActivity.DEFAULT_OCR_ENGINE_MODE).commit();

    // Autofocus
    prefs.edit().putBoolean(PreferencesActivity.KEY_AUTO_FOCUS, LPRCaptureActivity.DEFAULT_TOGGLE_AUTO_FOCUS).commit();
    
    // Beep
    prefs.edit().putBoolean(PreferencesActivity.KEY_PLAY_BEEP, LPRCaptureActivity.DEFAULT_TOGGLE_BEEP).commit();

    // Character blacklist
    prefs.edit().putString(PreferencesActivity.KEY_CHARACTER_BLACKLIST, 
        OcrCharacterHelper.getDefaultBlacklist(LPRCaptureActivity.DEFAULT_SOURCE_LANGUAGE_CODE)).commit();

    // Character whitelist
    prefs.edit().putString(PreferencesActivity.KEY_CHARACTER_WHITELIST, 
        OcrCharacterHelper.getDefaultWhitelist(LPRCaptureActivity.DEFAULT_SOURCE_LANGUAGE_CODE)).commit();

    // Page segmentation mode
    prefs.edit().putString(PreferencesActivity.KEY_PAGE_SEGMENTATION_MODE, LPRCaptureActivity.DEFAULT_PAGE_SEGMENTATION_MODE).commit();

    // Reversed camera image
    prefs.edit().putBoolean(PreferencesActivity.KEY_REVERSE_IMAGE, LPRCaptureActivity.DEFAULT_TOGGLE_REVERSED_IMAGE).commit();
    
    // Light
    prefs.edit().putBoolean(PreferencesActivity.KEY_TOGGLE_LIGHT, LPRCaptureActivity.DEFAULT_TOGGLE_LIGHT).commit();
  }
  ...
  void displayProgressDialog() {
    ...
    String ocrEngineModeName = getOcrEngineModeName();
    if (ocrEngineModeName.equals("Both")) {
      indeterminateDialog.setMessage("Performing LPR using Cube and Tesseract...");
    } else {
      indeterminateDialog.setMessage("Performing LPR using " + ocrEngineModeName + "...");
    }
    indeterminateDialog.setCancelable(false);
    indeterminateDialog.show();
  }
