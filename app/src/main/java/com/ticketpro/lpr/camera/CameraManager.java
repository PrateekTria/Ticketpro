...
public final class CameraManager {
	...
	public CameraMode cameraMode = CameraMode.OCR;
	...
	public synchronized Rect getFramingRect() {
		if (framingRect == null) {
			...
			if (this.cameraMode == CameraMode.LPR) {
				int width = TPApplication.getInstance().lastViewFinderWidth;
				int height = TPApplication.getInstance().lastViewFinderHeight;
	...
	public synchronized Rect getFramingRectInPreview() {
		if (framingRectInPreview == null) {
			...
			if (screenResolution.x < screenResolution.y) {
				...
				rect.bottom = rect.bottom * cameraResolution.x / screenResolution.y + 20;
	...
	public synchronized void adjustFramingRect(int deltaWidth, int deltaHeight) {
		if (initialized) {
			...
			if (this.cameraMode == CameraMode.LPR) {
				...
				TPApplication.getInstance().lastViewFinderHeight = newHeight;
				TPApplication.getInstance().lastViewFinderWidth = newWidth;
			} else {
				int newWidth = framingRect.width() + deltaWidth;
				int newHeight = framingRect.height() + deltaHeight;
				int leftOffset = (screenResolution.x - newWidth) / 2;
