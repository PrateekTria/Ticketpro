...
public final class PlanarYUVLuminanceSource extends LuminanceSource {
	...
	public PlanarYUVLuminanceSource(byte[] yuvData, int dataWidth, int dataHeight, int left, int top, int width,
			int height, boolean reverseHorizontal) throws IllegalArgumentException {
		super(width, height);
	...
	public byte[] getMatrix() {
		...
		// If the caller asks for the entire underlying image, save the copy and give
		// them the
		...
		// If the width matches the full width of the underlying data, perform a single
		// copy.
		if (width == dataWidth) {
	...
	public LuminanceSource crop(int left, int top, int width, int height) {
		return new PlanarYUVLuminanceSource(yuvData, dataWidth, dataHeight, this.left + left, this.top + top, width,
				height, false);
	}
